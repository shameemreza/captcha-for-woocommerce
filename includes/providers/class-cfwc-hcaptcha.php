 ...
 * hCaptcha Provider.
 *
 * Implementation of hCaptcha CAPTCHA service.
 ...
 * Hcaptcha class.
 *
 * Implements the hCaptcha provider.
 *
 * @since 1.0.0
 */
class Hcaptcha extends Abstract_Provider {
	 ...
	 * @var string
	 */
	protected $token_field = 'h-captcha-response';
	 ...
	 * @var string
	 */
	protected $verify_endpoint = 'https://hcaptcha.com/siteverify';
	 ...
	 * @var string
	 */
	protected $api_key_url = 'https://dashboard.hcaptcha.com/sites/new';
	...
	public function get_description() {
		return __( 'Privacy-focused alternative, challenge-based. 1M free requests/month.', 'captcha-for-woocommerce' );
	}
	...
	protected function render_widget( $form_type, $args ) {
		...
		$widget_id = 'cfwc-hcaptcha-' . esc_attr( $form_type );
		?>
		<div id="<?php echo esc_attr( $widget_id ); ?>"
			 ...
			 data-callback="cfwHcaptchaCallback"
			 data-expired-callback="cfwHcaptchaExpired"
	...
	public function test_connection( $site_key, $secret_key ) {
		...
		// hCaptcha keys are UUIDs.
		if ( ! preg_match( '/^[a-f0-9-]{36}$/i', $site_key ) ) {
			return $this->create_error(
				'invalid_site_key',
				__( 'Invalid site key format. hCaptcha site keys are UUIDs.', 'captcha-for-woocommerce' )
			);
		}
	...
	private function get_error_message( $error_codes ) {
		...
		$messages = array(
			...
			'invalid-or-already-seen-response' => __( 'CAPTCHA expired. Please complete the verification again.', 'captcha-for-woocommerce' ),
			'sitekey-secret-mismatch' => __( 'Server configuration error. Please contact the site administrator.', 'captcha-for-woocommerce' ),
		);
